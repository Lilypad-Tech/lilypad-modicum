...
We need an `src/js/.env` file like this:
...
You can get a new address using:
...
If this is the first time we have spun up the stack - we need to move some funds to our admin wallet address:
...
We then use hardhat to deploy the smart contract.

If we have previously stood up the stack and wish to start again - we should:
